...
public class TestDepartmentDao extends BaseMethodsTest{
    ...
    public void testGetAllDepartments() {
        ...
        int actualCountDepatment = departments.size();
        Assert.assertEquals("Actual result must be expected", actualCountDepatment, expectedCountDepartment);
    }

    public int getIdByName (String departmentName) {
        String query = "select id from departments where name = :p_name";
        ...
        List<Department> departments = namedParameterJdbcTemplate.query(query, sqlParameterSource, new RowMapper<Department>() {
            public Department mapRow(ResultSet resultSet, int i) throws SQLException {
                ...
                return department;
            }//стоит возвращать объект с непроинициализированным полем ?
        });
        if (departments.isEmpty()) {
            throw new IllegalArgumentException("Department with that name " + departmentName + " doesn't exist");
        }
